...
def fullFileSTT(input_dir, text_dir):
    os.makedirs(text_dir, exist_ok=True)

    for file_name in os.listdir(input_dir):
        input_file_path = os.path.join(input_dir, file_name)
        output_file_path = os.path.join(text_dir, os.path.splitext(file_name)[0]+'.txt')
        
        print(f"🔁 Preprocessing Audio {input_file_path}")
        with open(output_file_path, "w", encoding="utf-8") as f:
            print(f"📝 Created a empty file at {output_file_path}")
        language_code = "en-US"
        ...
        convert_to_text(input_file_path,  output_file_path, language_code, stt_model="stt_model")
...
if __name__ == "__main__":
    ...
    fullFileSTT("rawAudio", "textFiles")