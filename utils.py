import os, time, shutil, string, re, contractions, numpy as np
...
def clean_doc(doc):
    """
    takes a string and returns the the cleaned string
    """
    ...
    # remove stop words
    stops_free_text = " ".join([word for word in lemmatised_text.split() if len(word)>1 and word not in stop_words])
